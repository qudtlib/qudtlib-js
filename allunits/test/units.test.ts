...
test("Qudt.derivedUnitsFromExponentUnitPairs(Unit, number)", () => {
  ...
  let expected = [Units.M__PER__DEG_C__M, Units.PER__DEG_C, Units.PER__K]
    .map((u) => u.iri)
    .sort();
  ...
  expected = [Units.M, Units.M3__PER__M2, Units.M2__PER__M, Units.M__PER__RAD]
    .map((u) => u.iri)
    .sort();
  expect(result.map((u) => u.iri).sort()).toStrictEqual(expected);
});
...
test("Qudt.derivedUnitsFromExponentUnitPairs(string, number)[using Iris]", () => {
  expect(
    ...
    [Units.Kilo__FT3, Units.KiloL, Units.M3, Units.RT, Units.STR]
      .map((u) => u.iri)
      .sort()
  );
  expect(
  ...
  expect(
    ...
    toSortedStringList([Units.M__PER__DEG_C__M, Units.PER__K, Units.PER__DEG_C])
  );
...
test("Qudt.derivedUnitsFromExponentUnitPairs(string, number)[using labels]", () => {
  ...
  expect(
    ...
    [Units.M__PER__DEG_C__M, Units.PER__K, Units.PER__DEG_C]
      .map((u) => u.iri)
      .sort()
  );
  expect(
...
describe.each([
  ...
  [5, Units.CCY_EUR__PER__W__HR],
  [6, Units.W__PER__M3],
  [7, Units.L__PER__SEC__M2],
  [8, Units.CCY_EUR],
  [9, Units.CCY_EUR__PER__M2],
  [10, Units.M2__PER__M],
  [11, Units.KiloN__PER__M2],
  [12, Units.W__PER__M2],
  [13, Units.CD__PER__M2],
  [14, Units.CCY_EUR__PER__W],
  [15, Units.LUX],
...
test.each([
  [new Decimal(new Decimal(1).toPrecision(3)), Units.M, Units.FT.iri, 3.28, 2],
  [
    new Decimal(new Decimal("36").toPrecision(3)),
    Units.DEG_C.iri,
    Units.DEG_F,
    96.8,
    2,
  ...
  [
    new Decimal(new Decimal("100").toPrecision(5)),
    Units.DEG_F.iri,
    Units.DEG_C.iri,
    37.777,
    2,
...
test("Qudt.convertQuantityValue(QuantityValue, Unit)", () => {
  expect(
    Qudt.convertQuantityValue(
      new QuantityValue(new Decimal("1.00"), Units.M),
      Units.FT
    ).value.toNumber()
  ).toBeCloseTo(3.28, 2);
});
...
test("Unit.convert(Decimal, Unit)", () => {
  expect(Units.M.convert(new Decimal("1.00"), Units.FT).toNumber()).toBeCloseTo(
    3.28,
    2
  );
  expect(
...
describe.each([
  ...
  [
    Units.KiloGM,
    new FactorUnits([FactorUnit.ofUnit(Units.KiloGM)], new Decimal(1)),
  ],
...
test("Qudt.convert()[C to F] for TemperatureDifference", () => {
  expect(
    Qudt.convert(
      new Decimal(100),
      Units.DEG_C,
      Units.DEG_F,
      QuantityKinds.TemperatureDifference
    )
  ).toStrictEqual(new Decimal("180"));
});
test("Qudt.convert()[C to K] for TemperatureDifference", () => {
  expect(
    Qudt.convert(
      new Decimal(100),
      Units.DEG_C,
